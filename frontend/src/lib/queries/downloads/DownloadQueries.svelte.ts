...
// The summary is only a recovery/fallback signal; per-task SSE carries live
// progress. Sub-second polling keeps mobile radios and the server busy while a
// download is active, so use a modest cadence instead.
...
export const getAlbumDownloadsQuery = (
	...
	createQuery(() => ({
		...
		refetchInterval: (query: { state: { data?: DownloadListResponse | undefined } }) =>
			!usesMobileLowPowerVisuals() && hasActiveTask(query.state.data?.items ?? []) ? 2500 : false
	}));
