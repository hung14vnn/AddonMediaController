<script lang="ts">
	...
	const timedLines = $derived(
		lyricsQuery.data?.is_synced ? (lyricsQuery.data.lines ?? []).filter((line) => line.start_seconds !== null) : []
	);
	...
	$effect(() => {
		...
		// Do not use scrollIntoView here: it can also scroll the Listening Room page.
		...
		{#if lyricsOpen}
			<div class="absolute inset-0 flex flex-col overflow-hidden">
				<div class="flex items-center justify-between px-5 py-3">
					...
					{#if lyricsQuery.data?.is_synced}<span class="badge badge-xs badge-accent">Synced</span>{/if}
				</div>
				<div bind:this={lyricsViewport} class="flex-1 overflow-y-auto px-7 py-16" onscroll={pauseLyricsFollow}>
					{#if lyricsQuery.isFetching}
						<div class="flex h-full items-center justify-center"><span class="loading loading-spinner loading-md text-accent"></span></div>
					...
					{:else if timedLines.length}
						<div class="space-y-5">
							{#each timedLines as line, index (index)}
								<button type="button" data-lyric-line={index} onclick={() => playerStore.seekTo(line.start_seconds ?? 0)} class="block w-full text-left text-xl font-bold leading-tight transition-opacity sm:text-2xl {index === activeLineIndex ? 'text-accent' : index < activeLineIndex ? 'opacity-35' : 'opacity-55'}">{line.text}</button>
							{/each}
						</div>
					{:else if lyricsQuery.data?.text?.trim()}
						<pre class="whitespace-pre-wrap font-sans text-sm leading-relaxed text-base-content/80">{lyricsQuery.data.text}</pre>
					{:else}
						<p class="text-center text-sm text-base-content/50">Lyrics aren't available for this track.</p>
					{/if}
				</div>
			</div>
		{:else}
			<!-- Glow inset-0 must match the record's edge so the circles align. -->
			<div class="deck-halo absolute inset-0 -z-10 rounded-full" class:animate-glow-pulse={isPlaying}></div>
			<div class="turntable-platter vinyl-spin absolute inset-0 rounded-full" class:is-paused={!isPlaying}>
				<div class="pointer-events-none absolute inset-[9%] rounded-full border border-base-content/[0.06]"></div>
				<div class="pointer-events-none absolute inset-[18%] rounded-full border border-base-content/[0.07]"></div>
				...
				<div class="absolute inset-[48.5%] rounded-full bg-base-100 ring-1 ring-base-content/30"></div>
			</div>
			<div class="tonearm pointer-events-none absolute -right-1 -top-1 h-1/2 w-1/2" class:is-playing={isPlaying}>
				<div class="absolute right-[4.5%] top-[4.5%] h-4 w-4 rounded-full bg-base-300 ring-2 ring-base-content/20"></div>
				<div class="absolute right-[6.8%] top-[6.8%] h-1.5 w-[78%] origin-right rotate-[28deg] rounded-full bg-gradient-to-l from-base-content/40 to-base-content/15"><div class="absolute -left-1 top-1/2 h-2.5 w-2.5 -translate-y-1/2 rounded-sm bg-base-content/55 ring-1 ring-base-content/25"></div></div>
			</div>
		{/if}
	</div>
