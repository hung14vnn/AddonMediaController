...
class FileProcessor:
    ...
    async def _prepare_storage_source(
    ...
    ) -> tuple[Path, AudioInfo, Path | None]:
        """Prepare a deterministic AAC staging source without consuming FLAC early.
        ...
        if converted.exists():
            try:
                _tag, converted_info = await asyncio.to_thread(
                    self._tagger.read_tags, converted
                )
            except Exception:  # noqa: BLE001 - replace an incomplete prior artifact
                await asyncio.to_thread(converted.unlink, missing_ok=True)
        if converted_info is None:
        ...
        return converted, converted_info, source
    ...
    async def _place_matched_file(
        ...
        target_path = self._library_paths[0] / self._naming.format_path(
            manifest.naming_template,
            target_tag,
            self._storage_output_format(source, info),
        )
        ...
        for held in sorted(
            ...
            if (
                ...
                if present is not None and present.get("file_path") != str(target_path):
                    ...
                    replacement = present
            publish_source, publish_info, cleanup_source = (
                await self._prepare_storage_source(source, info)
            )
            planned.append(
                _PlannedImport(
                    source=publish_source,
    ...
    async def _process_one(
        ...
        return _PlannedImport(
            ...
            release_group_mbid=manifest.release_group_mbid,
            release_mbid=manifest.release_mbid,
            recording_mbid=manifest.external_track_id
            or (
                expected_track.recording_mbid
                if expected_track is not None
                else tag.musicbrainz_recording_id
